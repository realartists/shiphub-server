namespace RealArtists.ShipHub.Actors.GitHub {
  ...
  public abstract class AbstractActorPool : IGitHubPoolable {
    /// <summary>
    /// Removes an actor from the pool.
    /// </summary>
    protected abstract void Remove(long userId);

    /// <summary>
    /// Returns a new actor to use. May be random, a ring buffer, whatever.
    /// </summary>
    /// <returns>The actor to use next, or null if none are available.</returns>
    protected abstract IGitHubActor GetActor();

    /// <summary>
    /// Returns a specific actor by userId. Used when cache data is available.
    /// If the requested user is not in the pool, return null.
    /// </summary>
    /// <returns>The requested actor, or null if it is not in the pool.</returns>
    protected abstract IGitHubActor GetActor(long userId);
