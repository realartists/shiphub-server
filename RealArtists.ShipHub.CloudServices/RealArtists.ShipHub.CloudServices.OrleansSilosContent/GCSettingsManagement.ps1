...
if($configurationDocument -ne $null)
{
	if(Create-ElementStructureIfNotExists $configurationDocument)
	{
		...
		if($runtimeElement -ne $null)
		{
			...
			$serverGCElement.SetAttribute("enabled", $serverGC.ToString([System.Globalization.CultureInfo]::InvariantCulture))
			...
			$concurrentGCElement.SetAttribute("enabled", $concurrentGC.ToString([System.Globalization.CultureInfo]::InvariantCulture))
		}
	}
