CREATE TABLE [dbo].[Reactions]  (
  ...
  CONSTRAINT [CK_Reactions_IssueOrCommentOrPullRequestExclusive] CHECK (
    1 = (CASE WHEN IssueId IS NULL THEN 0 ELSE 1 END +
         ...
         CASE WHEN CommitCommentId IS NULL THEN 0 ELSE 1 END)),
  ...
  CONSTRAINT [FK_Reactions_CommitCommentId_CommitComments_Id] FOREIGN KEY ([CommitCommentId]) REFERENCES [dbo].[CommitComments] ([Id]),
)
GO
...
CREATE NONCLUSTERED INDEX [IX_Reactions_CommitCommentId]
  ON [dbo].[Reactions]([CommitCommentId])
  WHERE ([CommitCommentId] IS NOT NULL)
GO
